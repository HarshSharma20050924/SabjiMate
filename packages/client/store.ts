    ...
    (set, get) => ({
      ...
      fetchInitialList: async () => {
        ...
        try {
            ...
            // Only use standing order if wishlist is empty and not locked
            if (get().wishlist.length === 0 && standingOrder && standingOrder.length > 0 && !isLocked) {
                const initialList = standingOrder.map(item => ({ id: item.vegetableId, quantity: item.quantity }));
                set({ wishlist: initialList, isWishlistConfirmed: false });
                message = "Your Daily Essentials have been added to your list.";
            } else if (userWishlist && userWishlist.length > 0) {
                 // If there's a persisted wishlist from today, use that instead.
                const initialList = userWishlist.map(item => ({ id: item.vegetableId, quantity: item.quantity }));
                set({ wishlist: initialList, isWishlistConfirmed: true });
            }
            return message;
        } catch (error) {
      ...
      clearLocalWishlist: () => set({ wishlist: [], isWishlistConfirmed: false }),
    }),
    {
      name: 'sabzimate-wishlist-storage', // unique name
      storage: createJSONStorage(() => localStorage), // (optional) by default, 'localStorage' is used
      partialize: (state) => ({ wishlist: state.wishlist }), // Only persist the 'wishlist' part of the state
    }
  )
);